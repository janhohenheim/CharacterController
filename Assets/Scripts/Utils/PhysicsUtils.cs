...
namespace Utils
{
    ...
    public static class PhysicsUtils
    {
        ...
        public unsafe static NativeList<DistanceHit> ColliderDistanceAll(PhysicsCollider collider, float maxDistance, RigidTransform transform, ref CollisionWorld collisionWorld, Entity ignore, Allocator allocator = Allocator.TempJob)
        {
            ...
            var allDistances = new NativeList<DistanceHit>(allocator);
        ...
        public static NativeList<ColliderCastHit> ColliderCastAll(PhysicsCollider collider, float3 from, float3 to, ref CollisionWorld collisionWorld, Entity ignore, Allocator allocator = Allocator.TempJob)
        {
            unsafe
            {
                ...
                var allHits = new NativeList<ColliderCastHit>(allocator);
        ...
        public static NativeList<RaycastHit> RaycastAll(float3 from, float3 to, ref CollisionWorld collisionWorld, Entity ignore, CollisionFilter? filter = null, Allocator allocator = Allocator.TempJob)
        {
            var input = new RaycastInput()
            {
                Start = from,
                End = to,
                Filter = filter ?? PhysicsCollisionFilters.AllWithAll
            };

            var allHits = new NativeList<RaycastHit>(allocator);
        ...
        {
            for (var i = (castResults.Length - 1); i >= 0; --i)
            {
                if (entityManager.HasComponent<PhysicsCollider>(castResults[i].Entity))
                {
                    var collider = entityManager.GetComponentData<PhysicsCollider>(castResults[i].Entity);
        ...
        public unsafe static void TrimByFilter<T>(ref NativeList<T> castResults, ComponentDataFromEntity<PhysicsCollider> colliderData, CollisionFilter filter) where T : struct, IQueryResult
        {
            for (var i = (castResults.Length - 1); i >= 0; --i)
            {
                ...
                {
                    var collider = colliderData[castResults[i].Entity];
        ...
        public static bool GetSmallestFractional<T>(ref NativeList<T> castResults, out T nearest) where T : struct, IQueryResult
        {
            ...
            for (var i = 0; i < castResults.Length; ++i)
            {
